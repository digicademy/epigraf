...
This pattern applies to all entities in the [Relational Article Model (RAM)](/user/coreconcepts/model). The import file can simply consist of multiple `<project>`, `<article>`, `<section>`, `<item>`, `<footnote>`, `<link>` and `<property>` elements one after another. The element order is irrelevant.
