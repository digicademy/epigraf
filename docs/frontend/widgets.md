...
The widget HTML elements are generated in the [backend](/docs/backend/view) by helper classes derived from CakePHP helpers.
...
The HTML elements are generated by the collection views in the [backend](/docs/backend/view)
...
- **Subordinate** data contained within the main document widget or its satellite widgets
  comprise, for example, sections and items (see the [model layer in the backend](/docs/backend/model)).
