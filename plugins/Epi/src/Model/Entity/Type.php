...
class Type extends RootEntity
{
     ...
     * @return array
     */
    public function getShouldArray($key)
    {
        $shouldHave = [];
        foreach($this['merged'][$key] ?? [] as $shouldKey => $shouldConfig) {
            $shouldConfig = is_array($shouldConfig) ? $shouldConfig : ['type' => $shouldConfig, 'count' => '1'];
            if (!isset($shouldConfig['type'])) {
                $shouldConfig['type'] = $shouldKey;
            }

            if (!isset($shouldConfig['count'])) {
            ...
            $shouldConfig['has'] = 0;
            $shouldHave[$shouldConfig['type']] = $shouldConfig;
        }
        return $shouldHave;
    }
     ...
     *
     * The actual number has to be counted beforehand in the 'has' key.
     *
     * @param array $shouldHave An array generated by getShouldArray().
     * @param string $entityId An identify to output in the warning message, e.g. the ID of the entity.
     * @return array Return an array of warnings if the expected counts are not met.
     */
    public function evalShouldArray($shouldHave, $entityId)
    {
        $warnings = [];
        foreach ($shouldHave as $shouldType => $shouldConfig) {
            if (!isset($shouldConfig['count'])) {
                $warnings[] = ['msg' => __('Entity {0} should have no children of type {1}, but has {2}.', $entityId, $shouldType,  $shouldConfig['has'])];
            }
            elseif (($shouldConfig['count'] === '1') && ($shouldConfig['has'] !== 1)) {
                $warnings[] = ['msg' => __('Entity {0} should have exactly one child of type {1}, but has {2}.', $entityId, $shouldType, $shouldConfig['has'])];
            }
            elseif (($shouldConfig['count'] === '+') && ($shouldConfig['has'] < 1)) {
                $warnings[] = ['msg' => __('Entity {0} should have at least one child of type {1}, but has {2}.', $entityId, $shouldType, $shouldConfig['has'])];
            }
            elseif (($shouldConfig['count'] === '?') && ($shouldConfig['has'] > 1)) {
                $warnings[] = ['msg' => __('Entity {0} should have no more than one children of type {1}, but has {2}.', $entityId, $shouldType, $shouldConfig['has'])];
            }
        }
        return $warnings;
    }
