...
//                    $value = mb_strimwidth($value,0,40,'...');
     ...
     * - sort: true|false Defaults to true. Set to false to omit table header sort links.
     *                    For example, cursored collections such as property trees can not be sorted.
     *                    Set to 'multi' to enable multi column sorting.
    ...
    public function filterTable($model, $entities, $options=[])
    {
        ...
        if (empty($options['tree'])) {
            $options['tree'] = false;
        }  elseif (isset($options['collapsed'])) {
            $options['tree'] = $options['collapsed'] ? 'collapsed' : true;
        } else {
        ...
        if ($options['sort'] ?? true) {
            ...
            if (($options['sort'] ?? true) === 'multi') {
                $tableAttributes['data-sortmulti'] = '1';
            }
        }
    ...
    public function getTableHeader($model, $enitites, $options=[])
    {
        ...
        $collapseButton = $options['collapse'] ?? false;
        $collapse_all = $options['collapsed'] ?? false;

        $headerButtons = '';
        if ($collapseButton) {
            $headerButtons .= $this->collapseButton($group, $collapse_all);
        }
        if ($colSelector) {
            $headerButtons .= $this->columnSelector($group, $columns_all);
        }
    ...
    /**
     * Output the table body with all entities
     *
     * Each row is generated by getTableRow().
     *
     * @param string $model
     * @param array|ResultSet $entities
     * @param array $options
     * @return string
     */
    ...
    /**
     * Output children of an entity (e.g. sections in an article or fulltext search hits of an article)
     *
     ...
     *
     * Set the content option to:
     *
     * - ['details'] to show child rows
     * - ['search'] to show search results
     * - An array of field names with content to show in the child rows.
     *
     * @param $model
     * @param $entity
     * @param array $options
    ...
    public function getTableDetailRows($model, $entity, $options)  {
        ...
        if ($details === 'cursor') {
            ...
            // First column
            $indent = ($options['tree'] ?? false) ? $this->Tree->getIndentation($entity, $options['fold'] ?? 'foldable',
                'child') : '';
        ...
        foreach ($contentConfig as $contentField) {
            ...
            if ($contentField === 'search') {
                ...
                if ($contentField == 'details') {
                    $entityContent = $entity->getTree($options['targets'] ?? []);
                }
                else {
            ...
            foreach ($entityContent as $childRow) {
                ...
                if (empty($childRow) || (($childRow['id'] ?? '') === ('articles-' . $entity->id))) {
                    continue;
                }

                if ($contentField != 'details' || ($childRow['parent_id'] === ('articles-' . $entity->id))) {
                ...
                if ($contentField != 'details') {
                    $trAttributes['class'][] = 'row-supplement-parts';
                }
                ...
                $second_col_content = '';
                if ($contentField != 'details') {
                    $second_col_content = '<p>' . $childRow[$childColumns['content']] . '</p>';
                }
                $out .= $this->Element->outputHtmlElement(
                    'td',
                    $second_col_content,
    ...
    public function collapseButton($model, $collapsed = false)
    {
        $group = str_replace('.','_',$model);
        $classes = ['button-settings widget-filter-item widget-filter-item-collapse'];
        if ($collapsed) {
            $classes[] = 'active';
        }
        ...
        $out = '<button
            ...
            data-filter-group="' . $group . '"
            title="' . __('Expand / collapse all') . '"
            aria-label="' . __('Expand / collapse all') . '">⊟</button>';
...
//        $out .= $this->filterReset($group);
     ...
     *
     * See FilterWidget->updateResults() in filter.js for how the form action is generated from search parameters.
     *
     * @param string $group The filter group, an identifier that knits together different search inputs.
     * @param string $url The base URL for generating the form action.
     *
     * @return mixed
     */
