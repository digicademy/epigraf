...
<div class="content-extratight">
    <?= $this->Form->create($entity, ['id' => 'form-create-databanks-' . $entity->id]) ?>
    <fieldset>
        <?= $this->Form->hidden('id'); ?>
        <p>
            <?= __('This will create the database {0} on the server. Are you sure?', $entity->name) ?>
        </p>
    </fieldset>
    <?= $this->Form->end() ?>
</div>
...
<?php $this->Link->addAction(
    __('Cancel'),
    ['controller' => 'Databanks', 'action' => 'index'],
...
$this->Link->addAction(
    __('Yes'),
    [],
    ['linktype' => 'submit', 'form' => 'form-create-databanks-' . $entity->id]
);
?>
