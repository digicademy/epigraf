...
<xsl:stylesheet version="2.0" 
  ...
  <!-- 
    mit diesem stylesheet werden buchstabenverbindungen (ligaturen) 
    verarbeitet und formatiert  
  -->

  <!--buchstabenverbindungen-->
  <xsl:template match="all">
    <!-- ausgabeoptionen abfragen, ob buchstabenverbindungen mit 
      unterbögen gekennzeichnet oder nur unterstrichen werden sollen -->
    <xsl:choose>
      <!-- ausgabe mit unterbögen -->
      <xsl:when test="$sw_ligaturboegen='1'"><xsl:call-template name="ligatures" /></xsl:when>
      
      <!-- unterstreichung: die formatierung erfolgt aus dem textknoten heraus 
      in di-doc-commons.xsl und in di-doc-styles.xsl -->
      <xsl:otherwise><xsl:apply-templates /></xsl:otherwise>
    </xsl:choose>
   </xsl:template>

  <!-- erzeugen von ligaturbögen -->
  <xsl:template name="ligatures">
    <!-- parameter mit formatanweisungen,
         es wird der sonderfont Araldo benötigt-->
    <xsl:param name="p_rPr">
      <w:rFonts w:ascii="Araldo" w:h-ansi="Araldo"/>
      <wx:font wx:val="Araldo"/>
      <!-- die bögen müssen höhergestellt werden, damit sie in der zeile sichtbar sind -->
      <w:position w:val="2"/>
    </xsl:param>

    <!-- die buchstabenverbindungen nach typen unterscheiden -->
    <xsl:choose>
      <!--Ligatur und Bogenverschmelzung und Nexus litterarum;
          nur diese werden durch unterbögen gekennzeichnet-->
      <!-- TODO: Use IRIs or config -->
      <xsl:when test="
                      @lemma='fsn' or 
                      @lemma='Bogenverschmelzung' or 
                      ...
                      @lemma='Verschränkung' or 
                      @lemma='nex' or 
                      contains(@value,'Nexus') or 
                      contains(@data-link-value,'Nexus') or 
                      contains(@lemma,'Nexus') or 
                      @value='' or
                      @data-link-value=''
                      ">

        <!--erstes zeichen mit rechtsoffenem bogen (linker bogen) unterlegen-->
        <w:r>
          <w:rPr>
        ...
        <!--die zeichen einer verbindung zwischen dem ersten und dem letzten zeichen 
            werden mit flachen bogenverbindern (mit unterstrichen) unterlegt;
            in der verarbeitungsschleife wird jedes zeichen einzeln angesteuert
        -->
        <xsl:call-template name="loop_ligatures">
          <xsl:with-param name="p_Zaehler" select="2" />
          <xsl:with-param name="p_Ende" select="string-length()-1" />
          <xsl:with-param name="p_rPr"><xsl:copy-of select="$p_rPr"></xsl:copy-of></xsl:with-param>
        </xsl:call-template>
        
        <!--letztes zeichen mit nach links offenem bogen (rechter bogen) unterlegen-->             
        <xsl:choose>
      ...
      <!--sonstige buchstabenverbindungen werden nicht als solche gekennzeichnet-->
      <xsl:otherwise>
        <xsl:value-of select="."/>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <!-- bogenverbinder -->
  <xsl:template name="loop_ligatures">
    <xsl:param name="p_rPr" />  
    <xsl:param name="p_Zaehler" />
    <xsl:param name="p_Ende" />

    <!-- wenn in einer buchstenverbindung <all> das element <noAll> vorkommt, 
      handelt es sich um eine in der verbindung aufgelöste abkürzung,
      deren zeichen nicht mit unterbögen gekennzeichnet werden sollen;
      
      die auszeichnung mit <noAll> erfolgt in de transformationsstufe 2 im template 
      di-trans3-inschriften.xsl
    -->
    
        <!-- 
        die striche unter den zwischenbuchstaben (bogenverbinder) werden vor diese buchstaben gesetzt und 
        schieben sich linksbündig nach recht unter dieselben;
        im font Araldo gibt es vier unterschiedlich lange bogenverbinder (unterstriche):
        kurz 0042 = B
        länger 0045 = E
        noch länger 0048 = H
        am längsten 004B = K
        
        (die großbuchstaben geben die plätze im unicode-raster dieses fonts wieder)
        
        die nun folgenden formatanweisungen wählen für jeden buchstaben des alphabets 
        (und jedes zeichen) gemäß seiner breite einen passenden unterstrich aus,
        die buchstaben und zeichen gleicher länge sind dabei zu gruppen zusammengefasst
        -->
    <xsl:if test="$p_Zaehler &lt;= $p_Ende">
      <xsl:choose>
        <xsl:when test="substring(.,$p_Zaehler,1)= 'i' or 
                        substring(.,$p_Zaehler,1)= 'j' or
                        substring(.,$p_Zaehler,1)= ']' or
                        substring(.,$p_Zaehler,1)= '[' or
                        substring(.,$p_Zaehler,1)= '(' or 
                        substring(.,$p_Zaehler,1)= ')'
                              ">
          <!--<fo:character font-family="Araldo" character="c" keep-with-next.within-line="always" keep-with-previous.within-line="always"/>-->
          <w:r>
            <w:rPr>
        ...
        <xsl:when test="substring(.,$p_Zaehler,1)= ' ' ">
        ...
        <xsl:when test="substring(.,$p_Zaehler,1)= 'a' or 
                        substring(.,$p_Zaehler,1)= 'c' or
                        substring(.,$p_Zaehler,1)= 's' or
                        substring(.,$p_Zaehler,1)= 'S'
                        ">
          <!--<fo:character font-family="Araldo" character="&#x0045;" keep-with-next.within-line="always" keep-with-previous.within-line="always"/>-->
          <w:r>
            <w:rPr>
        ...
        <xsl:when test="substring(.,$p_Zaehler,1)= 'd' or 
                        substring(.,$p_Zaehler,1)= 'g' or
                        substring(.,$p_Zaehler,1)= 'm' or 
                        substring(.,$p_Zaehler,1)= 'q' or 
                        substring(.,$p_Zaehler,1)= 'w' or 
                        substring(.,$p_Zaehler,1)= 'o'">
        ...
        <xsl:when test="substring(.,$p_Zaehler,1)= 'A' or
                         substring(.,$p_Zaehler,1)= 'D' or
                         substring(.,$p_Zaehler,1)= 'G' or
                         substring(.,$p_Zaehler,1)= 'H' or
                         substring(.,$p_Zaehler,1)= 'K' or
                         substring(.,$p_Zaehler,1)= 'M' or
                         substring(.,$p_Zaehler,1)= 'N' or
                         substring(.,$p_Zaehler,1)= 'O' or
                         substring(.,$p_Zaehler,1)= 'Q' or
                         substring(.,$p_Zaehler,1)= 'U' or
                         substring(.,$p_Zaehler,1)= 'W' 
                        ">
          <!--<fo:character font-family="Araldo" character="&#x004B;" keep-with-next.within-line="always" keep-with-previous.within-line="always"/>-->
          <w:r>
            <w:rPr>
        ...
        <xsl:when test="substring(.,$p_Zaehler,1)= 'B' or
                         substring(.,$p_Zaehler,1)= 'C' or
                         substring(.,$p_Zaehler,1)= 'E' or
                         substring(.,$p_Zaehler,1)= 'F' or
                         substring(.,$p_Zaehler,1)= 'L' or
                         substring(.,$p_Zaehler,1)= 'P' or
                         substring(.,$p_Zaehler,1)= 'R' or
                         substring(.,$p_Zaehler,1)= 'T' or
                         substring(.,$p_Zaehler,1)= 'V' or
                         substring(.,$p_Zaehler,1)= 'X' or
                         substring(.,$p_Zaehler,1)= 'Y' or
                         substring(.,$p_Zaehler,1)= 'Z'
                         ">
   ...
   <!-- <fo:character font-family="Araldo" character="&#x0048;" keep-with-next.within-line="always" keep-with-previous.within-line="always"/>-->
      ...
      <w:r>
        <!-- befindet sich die buchstabenverbindung in einem zitat, 
          wird die entsprechende formatanweisung zusätzlich aufgerufen -->
        <w:rPr>
          <xsl:if test="ancestor::quot">
            <w:rStyle w:val="epi-zitat"/>
          </xsl:if>
        ...
        <!-- zeichen einfügen -->
        <w:t><xsl:value-of select="substring(.,$p_Zaehler,1)"/></w:t>
      </w:r>

      <!-- ligaturschleife für das nächst folgende zeichen aufrufen -->  
      <xsl:call-template name="loop_ligatures">
          <xsl:with-param name="p_rPr"></xsl:with-param>
