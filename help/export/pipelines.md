...
To transfer metadata into image files, you need to create an export pipeline
that puts all images into a ZIP-archive.
In the [item type configuration](../configuration/articles) for images you can explicitly define metadata input fields.
In the [export pipeline](pipelines.md) you use placeholder strings to map article data (including image items)
and project data to specific metadata attributes.
