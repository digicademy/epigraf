...
class HtmlView extends ApiView
{
    ...
    /**
     * The name of the layout file. The name
     ...
     */
    static protected $_extension = 'plain';
    ...
    public function initialize(): void
    {
        ...
        $this->loadHelper('Files.Files');
        $this->loadHelper('Widgets.Link');
        $this->loadHelper('Widgets.EntityHtml');

        $this->setPlugin('Epi');
        $this->Renderer = $this->EntityHtml;
    }
    ...
    public function renderContent($data, $options = [], $level = 0)
    {
        ...
        if (is_object($data) && ($data instanceof RootEntity)) {
            $value = $this->Renderer->render($data, $options);
        }
        ...
        elseif ($data instanceof ResultSet) {
            foreach ($data as $row) {
                if ((is_object($row) && ($row instanceof RootEntity))) {
                    $value .= $this->Renderer->render($row, $options);
                }
            }
        }
